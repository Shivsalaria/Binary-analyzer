...
function DashBoard() {
    ...
    // Scroll handler को update करें
    useEffect(() => {
        const output = outputRef.current;
        if (!output) return;

        const handleScroll = () => {
            const isScrollingUp = output.scrollTop < lastScrollPosition.current;
            lastScrollPosition.current = output.scrollTop;
    ...
    // typeText function को update करें
    const typeText = (text) => {
        let index = 0;
        const minDelay = 10;
        const maxDelay = 30;
    ...
    // Effect to reset auto-scroll when new data comes
    useEffect(() => {
        if (aiData) {
            setAutoScroll(true);
            setDisplayedOutput('');
                                ...
                                <div className="h-full bg-white p-4 overflow-auto">
                                    ...
                                    <div
                                        ...
                                        style={{
                                            ...
                                            maxHeight: '500px'  // Add a fixed height to ensure scrolling
                                        }}
                                    >
                                        {aiLoading ? (
